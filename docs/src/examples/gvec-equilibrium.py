...
def generate_gvec_equilibrium():
    """Solve GVEC and retain its final state and diagnostics for later post-processing."""
    import gvec

    from struphy.utils._gallery import merge_metadata

    gvec_directory.parent.mkdir(parents=True, exist_ok=True)
    run = gvec.run(
        gvec_parameters(), runpath=gvec_directory, quiet=True, redirect_gvec_stdout=True
    )
    merge_metadata(
        "gvec-equilibrium",
        gvecIterations=int(run.GVEC_iter_used),
        gvecFinalForce=float(run.max_force),
    )


def create_simulation(use_gvec=False) -> Simulation:
    """Build the saved GVEC geometry, or a cheap placeholder for gallery metadata."""
    if use_gvec:
        project = gvec_parameters()["ProjectName"]
        equilibrium = equils.GVECequilibrium(
            rel_path=False,
            param_file=str((gvec_directory / f"parameter_{project}_final.ini").resolve()),
            dat_file=str((gvec_directory / f"{project}_State_final.dat").resolve()),
            use_nfp=True,
            num_elements=mapping_elements,
            degree=mapping_degree,
        )
        equilibrium.gradB1 = MethodType(gvec_grad_b_1, equilibrium)
        simulation_domain = equilibrium.numerical_domain
        folder = "gvec_equilibrium"
    else:
        equilibrium = equils.HomogenSlab(B0z=1.0)
        simulation_domain = domains.Cuboid()
        equilibrium.domain = simulation_domain
        folder = "gvec_equilibrium_metadata"
...
# Metadata generation calls the default factory without executing GVEC. The
# placeholder is never run; the executable block requests the saved GVEC geometry.
simulation_details = {
...
def pproc(sim: Simulation):
    ...
    data_file = Path(sim.env.path_out) / "data/data_proc0.hdf5"
    ...
    merge_metadata(
        ...
        figures=figures,
        **export_profiling(sim, "gvec-equilibrium"),
    )
