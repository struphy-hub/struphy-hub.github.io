...
T_END, DT, SAVE_STEP = 13.0, 0.02, 5  # the trail is the whole run: 13 time units, 2.4 gyrations
...
def particle_meshes(namespace: dict, state_files: dict) -> dict[str, pv.PolyData]:
    ...
    equilibrium.gradB1 = MethodType(namespace["gvec_grad_b_1"], equilibrium)  # not provided by GVEC, see the example
    domain = equilibrium.numerical_domain

    def to_xyz(eta):
        eta = np.array(eta, dtype=float)
        eta[..., 1:] %= 1.0  # both angles are periodic; points outside [0, 1) would be dropped by the mapping
        return np.asarray(domain(eta.reshape(-1, 3), change_out_order=True)).reshape(eta.shape)

    def unit_b(eta):
        return np.array([np.ravel(equilibrium.unit_b_cart(*e, squeeze_out=True))[:3] for e in eta])
    ...
    n = 2 * N_MARKERS  # candidates: some ions would start beyond the magnetic axis, where the inversion fails
    center = np.column_stack([rng.uniform(*RHO, n), rng.uniform(0.0, 1.0, (n, 2))])  # logical
    ...
    for _ in range(6):
        jacobian = np.stack(
            [(to_xyz(ion + step * np.eye(3)[i]) - to_xyz(ion - step * np.eye(3)[i])) / (2 * step) for i in range(3)],
            axis=-1,
        )
    ...
    print(f"ions placed on their gyro-orbit: {valid.sum()} of {n} candidates; using {len(keep)}")
    center, ion, v_par, b_hat, b_abs, v_perp_vec = (a[keep] for a in (center, ion, v_par, b_hat, b_abs, v_perp_vec))
    ion[:, 1:] %= 1.0  # the angles are periodic; markers must be loaded inside [0, 1)
    count = len(keep)

    def simulate(model, folder: str, markers, **extra):
        model.kinetic_ions.set_markers(
            loading_params=LoadingParameters(Np=count, seed=1, specific_markers=tuple(markers)),
        ...
        run = Simulation(
            model=model,
            env=EnvironmentOptions(out_folders=tempfile.mkdtemp(), sim_folder=folder, save_step=SAVE_STEP),
    ...
    gc = GuidingCenter()
    gc.propagators.push_bxe.options = gc.propagators.push_bxe.Options(maxiter=100, tol=1e-8)
    gc.propagators.push_parallel.options = gc.propagators.push_parallel.Options(maxiter=100, tol=1e-8)
    gc.kinetic_ions.var.add_background(maxwellians.GyroMaxwellian2D(n=(1.0, None), B0=float(b_abs.mean())))
    ...
    fo_history = simulate(fo, "hero_fo", np.column_stack([ion, v_par[:, None] * b_hat + v_perp_vec]))

    alive = np.isfinite(gc_history).all(axis=(0, 2)) & np.isfinite(fo_history).all(axis=(0, 2))
    print(f"markers confined in both models: {alive.sum()} of {count}")
    shown = np.flatnonzero(alive)[:N_SHOWN]
    gc_xyz, fo_xyz = (
        np.stack([to_xyz(history[t, shown]) for t in range(len(history))]) for history in (gc_history, fo_history)
    )
    ...
    average = np.stack([fo_xyz[t : t + window].mean(0) for t in range(len(fo_xyz) - window)])
    gap = np.linalg.norm(average - gc_xyz[window // 2 : window // 2 + len(average)], axis=-1)
...
def render_poster(surface: pv.PolyData, particles: dict[str, pv.PolyData]) -> None:
    ...
    plotter.add_mesh(surface, scalars="B", cmap=cmap, clim=B_RANGE, opacity=0.45, show_scalar_bar=False)
    for key, color in (("gc", GC_COLOR), ("fo", FO_COLOR)):
        plotter.add_mesh(particles[f"{key}-trails"], color=color, opacity=0.9, show_scalar_bar=False)
        plotter.add_mesh(particles[f"{key}-particles"], color=color, show_scalar_bar=False, smooth_shading=True)
    b = surface.bounds
